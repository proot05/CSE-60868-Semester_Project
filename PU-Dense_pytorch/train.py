...
import numpy as np
import torch
import torch.nn as nn  # Use standard PyTorch layers instead of MinkowskiEngine
...
# ------------------ Helper Functions for Dense Conversion ------------------

def batched_coordinates(list_of_coords):
    """
    Mimics ME.utils.batched_coordinates by adding a batch index to each point.
    Each element in list_of_coords should be a numpy array of shape (N, d) (typically d=3).
    Returns a numpy array of shape (sum_i N_i, d+1) where the first column is the batch index.
    """
    batch_coords = []
    for i, coords in enumerate(list_of_coords):
        batch_idx = np.full((coords.shape[0], 1), i)
        batch_coords.append(np.hstack([batch_idx, coords]))
    return np.vstack(batch_coords)

def sparse_to_dense(coords, feats):
    """
    Converts sparse coordinate-feature data to a dense tensor.
    Args:
      coords: numpy array or torch tensor of shape (N, 4) where each row is [batch, x, y, z].
      feats: torch tensor of shape (N, C)
    Returns:
      A dense tensor of shape (B, C, D, H, W) where B is the number of batches and
      D, H, W are inferred from the maximum coordinates.
    Note:
      This is a simple voxelization approach that assigns the feature at each coordinate.
      If multiple points fall into the same voxel, later ones will overwrite earlier ones.
      You might need to implement a more robust aggregation (e.g., average or max) if needed.
    """
    ...
    D = int(spatial_coords[:, 0].max().item() + 1)
    H = int(spatial_coords[:, 1].max().item() + 1)
    W = int(spatial_coords[:, 2].max().item() + 1)
    dense = torch.zeros(B, feats.shape[1], D, H, W, dtype=feats.dtype, device=feats.device)
    # Simple loop over points (can be optimized if needed)
    for i in range(coords.shape[0]):
        b = coords[i, 0].item()
        d, h, w = spatial_coords[i].tolist()
        dense[b, :, d, h, w] = feats[i]
    return dense
...
def test2(model, test_dataloader, logger, writer, writername, step, test_pc_error, args, device, First_eval):
    ...
    for i in range(len(test_iter)):
        ...
        for j, pc in enumerate(parts_pc):
            # Replace ME.utils.batched_coordinates with our batched_coordinates helper.
            p = batched_coordinates([pc])
            p2 = batched_coordinates([parts_pc2[j]])
            # Create features: a column of ones.
            f = torch.from_numpy(np.ones((p.shape[0], 1))).float()
            # Replace ME.SparseTensor with our dense conversion.
            x1 = sparse_to_dense(p, f).to(device)
            
            with torch.no_grad():
                # Note: the model interface may need to be updated to work on dense tensors.
                out, out_cls, target, keep = model(x1, coords_T=p2, device=device, prune=True)
            
            # Assuming the model now returns dense outputs.
            # In the original code out.C represented coordinates from the sparse tensor; you need to
            # decide how to extract the corresponding point cloud from the dense output.
            out_l.append(out[:, 1:])  # Adjust extraction as needed.
            out_cls_l.append(out_cls)
        ...
        loss = crit(rec_pc_cls.squeeze(), rec_target.type(rec_pc_cls.dtype).to(device))
...
def test1(model, test_dataloader, logger, writer, writername, step, args, device):
    ...
    for i in range(len(test_iter)):
        ...
        # Replace ME.SparseTensor with dense conversion.
        x = sparse_to_dense(coords, feats).to(device)
        
        with torch.no_grad():
            out, out_cls, target, keep = model(x, coords_T=coords_T, device=device, prune=True)
        
        loss = crit(out_cls.squeeze(), target.type(out_cls.dtype).to(device))
...
if __name__ == '__main__':
    ...
    test_dataloader = make_data_loader(filedirs=filedirs[:int(args.num_test)],
                                        GT_folder=None,
                                        batch_size=args.batch_size, 
                                        downsample=args.downsample,
                                        shuffle=False, 
